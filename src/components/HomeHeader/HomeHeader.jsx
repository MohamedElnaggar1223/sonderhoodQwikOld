...
export default function HomeHeader() 
{
    ...
    const toggleDrawer = (anchor, open) => (event) => {
      ...
      const list = (anchor) => (
        ...
        >
          <List>
            {['Home', 'About', 'Blogs', 'Services', 'Contact Us', 'Request A Session'].map((text) => (
    ...
    let content = ["left"].map((anchor) => (
        <React.Fragment key={anchor}>
          <Button style={{ alignSelf: 'flex-start', margin: '5%', fontSize: '6.5vw', fontWeight: '600', color: 'var(--Sonder-olive)' }} onClick={toggleDrawer(anchor, true)}></Button>
          <SwipeableDrawer
            //@ts-ignore
            anchor={anchor}
            open={state[anchor]}
    ...
    return (
        <>{width ? content : <div className='HomeHeaderContainer'>
            <div className='HomeHeaderContent'>
                <div className='HomeHeaderAnchor'><Link href='/'>HOME</Link></div>
                <div className='HomeHeaderAnchor'><Link href='/About'>ABOUT</Link></div>
                <div className='HomeHeaderAnchor'><Link href='/Blogs'>BLOG</Link></div>
                <div className='HomeHeaderAnchor'><Link href='/Services'>OUR SERVICES</Link></div>
            </div>
            ...
            <div className='HomeHeaderContent'>
                <div className='HomeHeaderAnchor'><Link href='/ContactUs'>CONTACT US</Link></div>
                {/* <div className='HomeHeaderAnchor'><Link href='/BookASession'>BOOK A SESSION</Link></div> */}
                <div className='HomeHeaderAnchor'><Link href='/RequestASession'>REQUEST A BOOKING</Link></div>
            </div>
        </div>}
        </>
    )
...
export const HomeHeaderQwik = qwikify$(HomeHeader)